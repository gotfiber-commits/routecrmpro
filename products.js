...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        // GET /super-auth/setup - Create admin with password 'admin123' (TEMPORARY - REMOVE IN PRODUCTION)
        if (method === 'GET' && path === '/setup') {
            const newHash = await hashPassword('admin123');
            
            // Delete existing and create new
            await query('DELETE FROM super_admins WHERE username = $1', ['admin']);
            
            const result = await query(
                `INSERT INTO super_admins (username, email, password_hash, name, can_create_companies, can_delete_companies, can_impersonate, status)
                 VALUES ($1, $2, $3, $4, true, true, true, 'active') RETURNING username, email`,
                ['admin', 'admin@routecrmpro.com', newHash, 'Platform Administrator']
            );
...
async function handleLogin(event) {
    ...
    console.log('Admin found:', admin.username, 'Hash length:', admin.password_hash?.length);
    ...
    if (!validPassword) {
        ...
        console.log('Password mismatch. Input password would hash to:', correctHash);
        console.log('Stored hash is:', admin.password_hash);
        return error('Invalid credentials', 401);
    }
